...
@interface NGCQSingleSelectionAnswerCell(){
 ...
 IBOutlet NSLayoutConstraint *option3bottomConstarint;
 IBOutlet NSLayoutConstraint *option2bottomConstarint;
}

@end

@implementation NGCQSingleSelectionAnswerCell
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([[touch view] class] == [UIImageView class])
    {
            if(([touch view] == self.option1Icon)||([touch view] == self.option2Icon)||([touch view] == self.option3Icon)||([touch view] == self.option4Icon))
            {
                ...
                if([self.delegateForAnsweredOption respondsToSelector:@selector(checkMarkOptionPressed:andCell:)])
                [self.delegateForAnsweredOption checkMarkOptionPressed:(UIImageView*)[touch view] andCell:self];
...
-(void)configureCQSingleSelectionAnswerCell:(NSDictionary *)dictData{
    ...
    switch (optionsArray.count) {
        case 2:
            ...
            option2bottomConstarint.constant = 14;
            break;
        case 3:
            
            option3bottomConstarint.constant = 14;
            [self.contentView removeConstraint:option2bottomConstarint];
            break;
        default:
            break;
    }
