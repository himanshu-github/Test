...
- (void)cacheImage:(UIImage *)image
{
	if (!self.cancelled)
	{
        if (image && self.URL)
        {
            ...
            if ([self.URL isFileURL])
            {
                NSString *prefix = [[self.URL absoluteURL] path];
                NSString *prefix1 = [[NSBundle mainBundle] resourcePath];

                if ([prefix hasPrefix:prefix1])
                {
...
- (void)setImage:(UIImage *)image
{
    ...
    if (image != self.image && self.crossfadeDuration!=0.0)
    {
