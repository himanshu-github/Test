...
- (void)setDefaultSettings {
    ...
    self.rightMenuPanEnabled = YES;
    self.leftMenuPanEnabled = YES;
}
...
- (void)openLeftSideMenuCompletion:(void (^)(void))completion {
    ...
    [self setCenterViewControllerOffset:self.leftMenuWidth animated:NO completion:completion];
...
- (void)leftMenuWillShow {
    ...
    self.leftMenuViewController.view.transform = [self closeTransformForMenuView];
    [self.menuContainerView bringSubviewToFront:[self.leftMenuViewController view]];
    UIViewAnimationOptions options = UIViewAnimationOptionCurveEaseInOut;
    [UIView animateWithDuration:0.25 delay:0.0 options:options animations:^{
        self.leftMenuViewController.view.transform = CGAffineTransformIdentity;
        [self.centerViewController view].layer.transform = [self openTransformForView:[self.centerViewController view]];
...
- (void)rightMenuWillShow {
    ...
    [UIView animateWithDuration:0.25 delay:0.0 options:options animations:^{
        ...
        [self.centerViewController view].layer.transform = [self openRightTransformForView:[self.centerViewController view]];
...
- (void) setLeftSideMenuFrameToClosedPosition {
    ...
    leftFrame.size.height = SCREEN_HEIGHT;
...
- (void) setRightSideMenuFrameToClosedPosition {
    ...
    rightFrame.size.height = SCREEN_HEIGHT;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if([gestureRecognizer.view isEqual:[self.centerViewController view]] && [touch.view isKindOfClass:[UIButton class]]){
    ...
    if([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if ([touch.view isKindOfClass:[UIButton class]] && self.menuState == MFSideMenuStateClosed) {
            return NO;
        }
...
- (void) handleLeftPan:(UIPanGestureRecognizer *)recognizer {
    ...
    [self openTransformForView:[self.centerViewController view] withX:translatedPoint.x];
...
{
    self.leftMenuViewController.view.transform = transform;
    self.leftMenuViewController.view.center = (CGPoint) { CGRectGetMidX(self.view.bounds), CGRectGetMidY(self.view.bounds) };
    self.leftMenuViewController.view.bounds = self.view.bounds;
}
- (CGAffineTransform)closeTransformForRightMenuView
{
    float zoomScale =  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 0.5634f : 0.85f;
    UIOffset edgeOffset = (UIOffset) { .horizontal = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 18.0f : 0.0f };
    
    CGFloat transformSize = 1.0f / zoomScale;
    ...
    return CGAffineTransformTranslate(transform, (CGRectGetMidX(self.view.bounds)) - edgeOffset.horizontal, -edgeOffset.vertical);
...
- (CGAffineTransform)closeTransformForMenuView
{
    ...
    CGFloat transformSize = 1.0f / zoomScale;
    CGAffineTransform transform = CGAffineTransformScale(self.leftMenuViewController.view.transform, transformSize, transformSize);
    return CGAffineTransformTranslate(transform, 0.0f, edgeOffset.vertical);
    
}

- (CATransform3D)openTransformForView:(UIView *)view
{
    
    
    float zoomScale =  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 0.5f : 0.85f;
    UIOffset edgeOffset = (UIOffset) { .horizontal = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 1.0f : 0.0f };
    
    CGFloat transformSize = zoomScale;
    
    CALayer *layer = view.layer;
    CATransform3D rotationAndPerspectiveTransform = CATransform3DIdentity;
    rotationAndPerspectiveTransform = CATransform3DTranslate(rotationAndPerspectiveTransform, (CGRectGetMidX(view.bounds) + edgeOffset.horizontal)+30, -(edgeOffset.vertical), 200);
    rotationAndPerspectiveTransform = CATransform3DScale(rotationAndPerspectiveTransform, transformSize, transformSize, 1);
...
- (CATransform3D)openRightTransformForView:(UIView *)view
{
    float zoomScale =  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 0.5f : 0.85f;
    UIOffset edgeOffset = (UIOffset) { .horizontal = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 1.0f : 0.0f };
...
- (CGAffineTransform)openTransformForView:(UIView *)view withX:(float)x
{
    float zoomScale =  UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 0.5634f : 0.85f;
    UIOffset edgeOffset = (UIOffset) { .horizontal = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone ? 18.0f : 0.0f };
    ...
    CGAffineTransform newTransform = CGAffineTransformTranslate(view.transform, x, edgeOffset.vertical);
