...
+(NSString*)getClassControllerNameForEnum:(NSInteger)value{

    switch (value) {
        case JD_FROM_SRP_PAGE:
            return @"JD_FROM_SRP";
            break;
        case JD_FROM_RECOMMENDED_JOBS_PAGE:
            return @"JD_FROM_RECOMMENDED_JOBS";
            break;

        case JD_FROM_SAVED_OR_SHORTLISTED_JOBS_PAGE:
            return @"JD_FROM_SAVED_OR_SHORTLISTED_JOBS_PAGE";
            break;

        case JD_FROM_VIEWMORE_OR_JD_SIM_JOB:
            return @"JD_FROM_VIEWMORE_OR_JD_SIM_JOB";
            break;

        case JD_FROM_ACP_SIM_JOB_PAGE:
            return @"JD_FROM_ACP_SIM_JOB_PAGE";
            break;
...
+ (NSString *)getFormattedTelephone:(NSString *)telephoneStr{
    NSString *str = @"";
    str = [telephoneStr stringByReplacingOccurrencesOfString:@"+" withString:@" "];
    return str;
}
...
+ (NSString *)getProfilePhotoPathForSocialLogin{
    NSString *docDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) fetchObjectAtIndex:0];
    NSString *pngFilePath = [NSString stringWithFormat:@"%@/%@",docDir,USER_PROFILE_PHOTO_NAME_FROM_SOCIAL_LOGIN];
...
-(float)getDynamicHeightWithFont:(UIFont *)font width:(float)width{
    CGSize maximumLabelSize = CGSizeMake(width,MAXFLOAT);
    CGRect labelRect = [self boundingRectWithSize:maximumLabelSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil];
